...
function refineMedication(med) {
  ...
  if (
    ...
    (/syringes/i.test(desc) && /mastitis|udder|teat|cloxacillin|amoxicillin|cefquinome|ampicillin/i.test(notes + ' ' + generic))
  ) {
    route = 'Intramammary';
    form = 'intramammary infusion';
    if (unit === 'mg/tab' || unit === 'mg/ml') unit = 'mg/syringe';
    return { id: med.id, brand, form, route, unit, val, reason: 'Intramammary' };
  }

  // -------------------------------------------------------------
  // 2. INTRAUTERINE (Oblets, flushes, pessaries)
  // -------------------------------------------------------------
  if (
    /\b(intrauterine|intra-uterine|uterine flush|oblet for intrauterine|post-partum uterine|metritis)\b/i.test(fullText) &&
    ...
    return { id: med.id, brand, form, route, unit, val, reason: 'Intrauterine' };
  }
  ...
  if (/\b(ophthalmic|eye drop|eye drops|eye ointment|conjunctival)\b/i.test(fullText)) {
    ...
    return { id: med.id, brand, form, route, unit, val, reason: 'Ophthalmic' };
  }
  ...
  if (/\b(ear drops|instill in ear|auricular|ear canal)\b/i.test(fullText) || (/\botic\b/i.test(fullText) && !/antibiotic|osmotic|probiotic/i.test(fullText))) {
    ...
    return { id: med.id, brand, form, route, unit, val, reason: 'Otic' };
  }

  // -------------------------------------------------------------
  // 5. POUR-ON / SPOT-ON / TOPICAL (Washes, sprays, dips, ointments)
  // -------------------------------------------------------------
  if (/\b(pour-on|pour on|spot-on|spot on)\b/i.test(desc + ' ' + notes + ' ' + brand)) {
    route = 'Pour-on';
    form = 'pour-on';
    if (unit === 'mg/tab') unit = 'mg/ml';
    return { id: med.id, brand, form, route, unit, val, reason: 'Pour-on' };
  }
  ...
  if (
    ...
    return { id: med.id, brand, form, route, unit, val, reason: 'Topical Ointment/Cream' };
  }

  if (
    (/\b(spray|wound spray|healing promoter|antiseptic wash|plunge dip|dip treatment|wash or spray|bathing|cutaneous spray)\b/i.test(desc + ' ' + notes + ' ' + brand)) &&
    !/vial|ampoule|solution for injection/i.test(desc)
  ) {
    route = 'Topical';
    form = /spray/i.test(desc + ' ' + brand) ? 'spray' : 'topical solution';
    if (unit === 'mg/tab') unit = 'mg/ml';
    return { id: med.id, brand, form, route, unit, val, reason: 'Topical Spray/Wash' };
  }

  // -------------------------------------------------------------
  // 6. INJECTABLE (Vials, Ampoules, Solutions/Suspensions for injection, Vaccines)
  // -------------------------------------------------------------
  const isVialOrAmp = /\b(vial|vials|ampoule|ampoules|amp\b|injection|injectable|solution for injection|suspension for injection|sterile solution|sterile suspension|freeze-dried vaccine|inactivated.*vaccine|live.*vaccine)\b/i.test(desc + ' ' + notes + ' ' + brand);
  const hasInjectionRouteInFreq = /\b(im\/iv|iv\/im|iv\/im\/sc|im\/sc|iv\/sc|im|iv|sc|deep im|slow iv)\b/i.test(freq);
  const isInjectableForm = origForm === 'injectable' && !/bottle|sachet|powder|paste/i.test(desc);

  if (isVialOrAmp || hasInjectionRouteInFreq || isInjectableForm) {
    form = 'injectable';
    if (unit === 'mg/tab' || unit === 'mg/5ml') unit = 'mg/ml';

    // Determine exact route from text
    ...
    return { id: med.id, brand, form, route, unit, val, reason: 'Injectable' };
  }

  // -------------------------------------------------------------
  // 7. ORAL FORMULATIONS (PO)
  // -------------------------------------------------------------
  route = 'PO';
  ...
  if (/\b(paste|oral paste)\b/i.test(desc + ' ' + notes + ' ' + brand)) {
    ...
    return { id: med.id, brand, form, route, unit, val, reason: 'Oral Paste' };
  }
  ...
  if (/\b(bolus|boluses|oblet|oblets|4 bolus)\b/i.test(desc + ' ' + notes + ' ' + brand)) {
    ...
    return { id: med.id, brand, form, route, unit, val, reason: 'Oral Bolus' };
  }

  // Powders / Sachets
  if (/\b(sachet|sachets|powder|water soluble powder|soluble powder)\b/i.test(desc + ' ' + notes + ' ' + brand)) {
    form = 'oral powder';
    if (unit === 'mg/ml' || unit === 'mg/tab') unit = 'mg/g';
    return { id: med.id, brand, form, route, unit, val, reason: 'Oral Powder' };
  }
  ...
  if (/\b(bottle|1l|500 ml|490 ml|2.2l|200 ml|120 ml|syrup|drench|oral drench|oral suspension|oral solution|drinking water)\b/i.test(desc + ' ' + notes + ' ' + brand)) {
    ...
    return { id: med.id, brand, form, route, unit, val, reason: 'Oral Liquid/Drench' };
  }
  ...
  if (/\b(capsule|capsules|cap\b)\b/i.test(desc + ' ' + notes + ' ' + brand)) {
    ...
    return { id: med.id, brand, form, route, unit, val, reason: 'Oral Capsule' };
  }

  if (/\b(tablet|tablets|tab\b)\b/i.test(desc + ' ' + notes + ' ' + brand)) {
    form = 'tablet';
    if (unit === 'mg/ml') unit = 'mg/tab';
    return { id: med.id, brand, form, route, unit, val, reason: 'Oral Tablet' };
  }

  return { id: med.id, brand, form, route, unit, val, reason: 'Oral Default' };
}

// Inspect results for all 1043 medications
const refined = meds.map(refineMedication);
const changedList = refined.filter((r, idx) => {
  const m = meds[idx];
  return m.route !== r.route || m.dosage_form !== r.form || m.concentration_unit !== r.unit;
});

console.log(`Total medications: ${meds.length}`);
console.log(`Total changed: ${changedList.length}`);

// Group by new route & form
const routeCounts = {};
const formCounts = {};
refined.forEach(r => {
...
console.log('\nFinal Route Counts:', JSON.stringify(routeCounts, null, 2));
console.log('\nFinal Form Counts:', JSON.stringify(formCounts, null, 2));
